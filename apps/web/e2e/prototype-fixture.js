...
export async function prototypeFixture(page, role = "staff") {
  ...
  const state = { coverageDelay: 0, failCoverage: false, failMonth: false, failExpense: false, failAnnual: false, failSave: false, expenseTotal: 360, expenseDelay: 0, unassigned: [], writes: [] };
  await page.route(/\/api\/(print-transactions|expense|dashboard)\b/, async (route) => {
    ...
    if (key === "expense/unassigned-devices") return json({ devices: state.unassigned });
    ...
    if (key === "dashboard/by-department") return state.failExpense ? json({}, 503) : json({ divisions: [{ id: 1, name: "ฝ่ายการพยาบาล", total_cost: state.expenseTotal, total_cost_satang: state.expenseTotal * 100, total_pages: 800, departments: [{ id: 1, name: "หน่วยบริการผู้ป่วยนอก", total_cost: 360, total_pages: 800, devices: [{ ...assets.rows[0], total_cost: 360, total_pages: 800, monthly: [{ month: "2026-09", net_pages: 800, total_cost: 360 }] }] }] }], unassignedDevices: [] });
    if (key === "dashboard/monthly-kpi") return json([{ month: "2026-09", total_pages: 800, total_cost: 360 }]);
    return json([]);
  });
  return state;
}
