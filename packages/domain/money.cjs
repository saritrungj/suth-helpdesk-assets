...
// ส่วนหน้าสุทธิคือ pages x 0.98 ซึ่งมีทศนิยมได้สองตำแหน่ง จึงคิดเป็น "ร้อยเท่าของหน้า"
// แล้วหารร้อยตอนท้ายครั้งเดียว ทุกขั้นเป็นจำนวนเต็มล้วน
//
//   สตางค์ = ปัดครึ่งขึ้น( pages x 98 x ราคาต่อหน้าเป็นสตางค์ / 100 )
//
// ## จุดปัดเศษมีจุดเดียว: "ทีละรายการยอดพิมพ์"
//
// ค่าใช้จ่ายถูกปัดเป็นสตางค์ตั้งแต่ระดับ **หนึ่งเครื่อง หนึ่งเดือน** แล้วการรวม
// ทุกระดับหลังจากนั้น (ต่อแผนก ต่ออาคาร ต่อสัญญา ต่อปีงบ) เป็นการบวกจำนวนเต็มล้วน
// ตรงกับวิธีที่ผู้ตรวจสอบคำนวณมือ คือคิดทีละบรรทัดแล้วค่อยบวก
//
// ⚠️ กฎนี้ผูกกับ SQL view ใน database/schema.sql ด้วย ไม่ใช่แค่ไฟล์นี้
//
// view `v_monthly_kpi`, `v_summary_by_building` และ `v_compare_usage_costs` ต้องมี
// `ROUND(..., 2)` คร่อมค่าใช้จ่ายของแต่ละแถวเสมอ ถ้าถอดออกเมื่อไหร่ SQL จะกลายเป็น
// การรวมค่าที่ยังไม่ปัดแล้วค่อยปัดครั้งเดียวตอนท้าย ซึ่งให้ผลต่างจากที่นี่ระดับ
// เศษสตางค์เมื่อรวมหลายร้อยรายการ
//
// เคยต่างกันจริงมาแล้ว: ยอดรวมของ /dashboard/monthly-kpi กับ /dashboard/summary-by-building
// ต่างกัน 2 สตางค์บนข้อมูลชุดเดียวกัน จับได้ด้วยเทส E2E ที่ต่อฐานข้อมูลจริง
// (`report-workflow.spec.js` หัวข้อ historical report views preserve … totals)
// ซึ่งจะแดงอีกครั้งถ้ามีใครถอด ROUND ออก

/** สัดส่วนหน้าที่คิดเงินจริง หลังหัก 2% (98/100) */
const BILLABLE_NUMERATOR = 98;
const BILLABLE_DENOMINATOR = 100;
...
/**
 * หน้าที่คิดเงินจริงหลังหัก 2%
 * @param {number|string} pages จำนวนหน้าดิบจากมิเตอร์
 * @returns {number} หน้าสุทธิ (มีทศนิยมได้สองตำแหน่ง)
 */
...
function costSatangAt(pages, priceSatang) {
  ...
  // ปัดครึ่งขึ้นตอนหารร้อย ครั้งเดียวตอนท้าย
