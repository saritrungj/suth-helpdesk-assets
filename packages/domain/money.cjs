...
//
// ## ยอดเงินจริงปัดที่ระดับรายการราคาต่องวด — ใน SQL ไม่ใช่ที่นี่
//
// ตั้งแต่ ADR-0022 ยอดที่เรียกเก็บคือ `ROUND(ราคา × Σ หน้าสุทธิ, 2)` ต่อ (งวด สัญญา
// หมวด ราคา) แล้วกระจายสตางค์ลงแต่ละมิเตอร์แบบ largest remainder ใน view
// `v_monthly_kpi` (database/schema.sql) ยอดย่อยทุกระดับ (เครื่อง แผนก อาคาร เดือน)
// จึงบวกกลับได้เท่าใบแจ้งหนี้ทุกสตางค์ การรวมยอดที่ได้จาก view ให้ใช้ `sumCostSatang`
// (บวกเป็นสตางค์ และพาจำนวนรายการที่หาราคาไม่ได้มาด้วย)
//
// `costSatang` / `costSatangAt` คิดเงินทีละรายการแล้วปัดทันทีตามกติกาเดิมของ ADR-0017
// ใช้ได้กับการประมาณ แต่**ห้าม**ใช้สร้างยอดเงินที่ต้องตรงใบแจ้งหนี้ เพราะต่างจาก view
// ได้ระดับเศษสตางค์
