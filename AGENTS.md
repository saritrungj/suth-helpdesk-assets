...
| `audit/` | อ่านประวัติการแก้ไข (ผู้ดูแล) — การเขียนบันทึกทำผ่าน `shared/audit.js` จากทุกเส้นทางเขียน ([ADR-0035](docs/decisions/0035-audit-log.md)) |
...
**ห้ามเปิดทางเข้าที่สองเข้าฐานข้อมูล** — ทุกอย่างที่อ่านหรือเขียนข้อมูลต้องผ่าน HTTP API เดิม เพราะกฎสิทธิ์อย่าง "viewer แก้ข้อมูลไม่ได้" อยู่ในชั้น API ทั้งหมด การต่อ MySQL ตรงจากเครื่องมืออื่นแปลว่าต้องเขียนกฎเดิมซ้ำแล้วรอวันที่มันไม่ตรงกัน — เคยมี `apps/mcp/` เป็นตัวอย่างที่ทำถูกข้อนี้ ถอดออกแล้วตาม [ADR-0012](docs/decisions/0012-remove-mcp-server.md) ข้อยกเว้นเดียวคือ harness ของฐาน QA แยกที่ต่อตรงได้เฉพาะตรวจตัวตน sentinel และหมุนรหัสตอน bootstrap ที่อนุมัติแล้ว ข้อมูลธุรกิจยังต้องผ่าน API — ขอบเขตอยู่ใน [ADR-0016](docs/decisions/0016-isolated-qa-database-and-bootstrap-harness.md) ซึ่ง [ADR-0024](docs/decisions/0024-docker-development-database.md) ขยายให้ครอบ `npm run db:bootstrap` ของฐานพัฒนาบน Docker (ตั้งรหัสบัญชีเท่านั้น)

**เครื่องมือที่ให้ AI เรียกได้ ต้องอ่านอย่างเดียว** — ถ้าวันหลังต่อผู้ช่วย AI เข้าระบบอีก (MCP หรืออะไรก็ตาม) ห้ามมีเครื่องมือที่เขียนข้อมูลเด็ดขาด ข้อมูลขาเข้าของเครื่องมือเหล่านี้มาจากโมเดลภาษา **ไม่ได้มาจากผู้ใช้ที่กดยืนยัน** ข้อความในเอกสารหรือไฟล์ที่โมเดลอ่านเจอระหว่างทางจึงกลายเป็นคำสั่งแก้ข้อมูลได้ ([NSA/CISA — MCP Security Design](https://media.defense.gov/2026/Jun/02/2003943289/-1/-1/0/CSI_MCP_SECURITY.PDF) ระบุให้ถือว่าข้อมูลขาเข้าของเครื่องมือเชื่อไม่ได้เสมอ) ยอดพิมพ์คือฐานของทุกบาทในระบบนี้ การบันทึกจึงต้องผ่านหน้าเว็บที่มีคนกดยืนยันเสมอ — จะผ่อนข้อนี้ต้องแก้ [ADR-0012](docs/decisions/0012-remove-mcp-server.md) พร้อมเหตุผลก่อน
...
- **kebab-case ตัวพิมพ์เล็ก** สำหรับไฟล์และโฟลเดอร์ทั่วไป — `run-docker-database.md`, `print-usage/`
...
- ชื่อต้องบอกบทบาท ไม่ใช่สถานะ — `import-format.md` ไม่ใช่ `final_v2.md`
...
### Finish an approved issue

เมื่อผู้ใช้สั่งเดินงานจนพร้อมส่งมอบ ใช้ [finish-issue](.agents/skills/finish-issue/SKILL.md): แก้–ทดสอบ–รีวิวต่อในขอบเขตที่อนุมัติ ไม่หยุดส่งรายงานทุก phase; หยุดเฉพาะครบเกณฑ์หรือมี blocker จริง คำสั่ง review-only และ approval gates ข้างต้นยังมีผลเสมอ
