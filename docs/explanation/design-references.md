...
| [GOV.UK — Table](https://design-system.service.gov.uk/components/table/) | caption และ `scope` ทุกตาราง | prop `caption` (เป็น `<caption>` และหัวเรื่องตอนขยาย) กับ `scope="col"` ใน `ui/UiDataTable.vue`; ใส่ caption แล้วที่ `views/AssetList.vue`, `views/PrintTransactions.vue` |

ตารางอื่นที่ยังไม่ส่ง `caption` (อันดับรายเครื่องใน `views/ByDepartment.vue` และหน้าตั้งค่า) ยังไม่ครบตามข้อ GOV.UK — ตามเก็บใน #51 หลังผลตรวจรอบที่ 3
