...
| [NN/g — Data tables](https://www.nngroup.com/articles/data-tables/), [NHS — Table](https://service-manual.nhs.uk/design-system/components/table) | hover และแถวสลับสีช่วยไล่แถว; แผงแก้ไขไม่ควรบังข้อมูลอ้างอิง | `--row-hover` / `--row-selected` / `--row-stripe` / `--scrim-panel` ใน `design/tokens.css`; แถวสลับสีและคอลัมน์แรกปักหมุดใน `ui/UiDataTable.vue`; กลุ่มฟอร์มเป็นการ์ดขาวบนพื้นแผงใน `components/DeviceFormFields.vue` (`grouped`) และ `views/MoveDeviceModal.vue` |
| [Carbon — Data table](https://github.com/carbon-design-system/carbon-website/blob/main/src/pages/components/data-table/usage.mdx), [Pagination](https://github.com/carbon-design-system/carbon-website/blob/main/src/pages/components/pagination/usage.mdx) | เครื่องมือตารางเป็นไอคอนได้ไม่เกินห้าปุ่ม; ตัวแบ่งหน้าอยู่ใต้ตารางเสมอ | ปุ่มขยายเป็นไอคอน และจำนวน + ตัวแบ่งหน้าอยู่ท้ายตารางใน `ui/UiDataTable.vue`; เต็มจอเฉพาะตารางข้อมูลยาว ส่วนรายงานทั้งก้อนและตารางสั้นไม่มีปุ่มขยาย |
...
### งานที่ต้องติดตาม — ลิ้นชักเดียว ไม่ใช่สองที่ (16 กันยายน 2026)

รายการงานที่ต้องติดตามเคยขึ้นทั้งบนแดชบอร์ดและในลิ้นชักแจ้งเตือน ด้วยสองชื่อ
("สิ่งที่ต้องจัดการ" กับ "งานที่ต้องติดตาม") ตอนนี้เหลือที่เดียวคือลิ้นชัก (#83)

| แหล่ง | ที่มา | เอามาใช้ที่ |
|---|---|---|
| [Carbon — Notification pattern](https://carbondesignsystem.com/patterns/notification-pattern/) | notification panel เหมาะกับข้อความที่ระบบสร้างเองและต้องย้อนกลับมาดูได้; จัดกลุ่มตามความเร่งด่วนหรือแหล่งที่มา; actionable notification มีปุ่มเดียวและคำบนปุ่มสั้น | `components/AttentionPanel.vue` — จัดกลุ่มตามความด่วน หัวกลุ่มบอกจำนวน และหนึ่งรายการมีปุ่มเดียว |
| [Linear — Inbox](https://linear.app/docs/inbox) | แยกของที่ต้องรีบ (Priority) ออกจากที่เหลือ (Other) แทนรายการเรียงยาวรายการเดียว | `components/AttentionPanel.vue` — สามกลุ่มเรียงจากด่วนที่สุด แทนการให้ผู้ใช้ตัดสินความด่วนเองทีละแถว |

**ข้อควรระวังที่ Carbon ระบุไว้และระบบนี้ไม่มี:** notification panel ปกติใช้คู่กับ toast
เพื่อบอกว่ามีของใหม่เข้ามา ระบบนี้ไม่มี toast ป้ายจำนวนบนกระดิ่งจึงเป็นสัญญาณเดียว
ที่เหลือ และต้องเปลี่ยนเป็นโทน danger เมื่อข้างในมีเรื่องระดับ "ต้องแก้ทันที"
(`app/AppNotifications.vue`) ไม่ใช่บอกแค่จำนวน
