...
| `JWT_SECRET` | ใช่ | กุญแจเซ็น JWT — ค่าสุ่มอย่างน้อย 32 ตัวอักษร ถ้าไม่ตั้ง สั้นกว่านั้น หรือเป็นค่าใน `.env.example` API จะไม่เปิด |
...
- token มีอายุ 8 ชั่วโมง ทุกคำขออ่านบัญชีปัจจุบันจากฐาน (`auth/current-user.js`, #208): บัญชีที่ถูกลบใช้ token เดิมไม่ได้ทันที ลดสิทธิ์มีผลทันที (เพิ่มสิทธิ์มีผลเมื่อล็อกอินใหม่) และเปลี่ยนรหัสผ่านแล้ว token ที่ออกก่อนหน้าใช้ไม่ได้ การเปลี่ยน `JWT_SECRET` ยังเป็นวิธีบังคับให้ **ทุกคน** ล็อกอินใหม่พร้อมกัน
